import torch
import numpy as np
import matplotlib.pyplot as plt
import lightning as L
from torch import nn, optim, utils
from torch.utils.data import Dataset, DataLoader, random_split
import opt_einsum

from Functions_and_classes import integrate, relative_l2_loss

class VlasovModel(L.LightningModule):
    def __init__(self, input_dim=4, Nv=32, Nt=32, N_basis_t=30,N_basis_v=30, basis_matrix=None):
        super().__init__()
        self.output_dim = Nt * Nv  # 256
        self.Nt = Nt
        self.Nv = Nv
        self.basis_matrix = basis_matrix
        self.N_basis_t = N_basis_t
        self.N_basis_v = N_basis_v
        
        # Basis setup
        if basis_matrix is None:
            raise ValueError("Must provide precomputed basis_matrix")
        self.K = basis_matrix.shape[0]  # Number of basis functions
        
        # Register basis as a fixed buffer (not trained)
        # For complex solutions, we use the SAME real basis for both real/imag parts
        self.register_buffer('basis_real', basis_matrix)  # (K, 256)
        self.register_buffer('basis_imag', torch.zeros_like(basis_matrix))  # No imaginary basis
        
        # Branch network: outputs K complex coefficients (2K reals)
        total_input = input_dim
        
        self.branch = nn.Sequential(
            nn.Linear(total_input, 512),
            nn.ReLU(),
            nn.Linear(512, 1024),
            nn.ReLU(),
            nn.Linear(1024,1024),
            nn.ReLU(),
            nn.Linear(1024, self.K *self.K * 2)  # K complex = 2K reals
        )
        
        self.loss_fn = nn.MSELoss()

    def compute_d_n(self, initial_conditions):
        """
        Projects initial condition onto basis at t=0.
        d_n = integral over v of psi_n(v, t=0) * f(v, t_i) dv
        Returns: (Batch, K) complex
        """
        batch_size = initial_conditions.shape[0]

        v = torch.linspace(-4.0, 4.0, self.Nv, device=self.device)
        dv = v[1] - v[0]
        w_v = torch.ones(self.Nv, device=self.device) * dv
        w_v[0] *= 0.5
        w_v[-1] *= 0.5  # (Nv,) trapezoidal weights

        # basis at t=0: (K, Nv)
        basis_3d = self.basis_real.reshape(self.K, self.Nt, self.Nv)
        basis_at_t0 = basis_3d[:, 0, :]  # (K, Nv)

        # initial_conditions: (Batch, Nv) complex
        # d_n[b, k] = sum_v w_v * psi_k(v) * f_0[b, v]
        f0 = initial_conditions.view(batch_size, self.Nv)  # (Batch, Nv)
        d_n = (basis_at_t0 * w_v).to(f0.dtype) @ f0.T  # (K, Batch)
        return d_n.T  # (Batch, K)
    
    def forward(self, params, initial_conditions):
        batch_size = params.shape[0]
        
        # Flatten initial conditions and split real/imag
        init_flat = initial_conditions.view(batch_size, -1)
        init_real_imag = torch.cat([init_flat.real, init_flat.imag], dim=1)
        
        # Concatenate params and initial conditions
        x = params
        t = torch.linspace(0.0, 10.0, self.Nt, device=self.device)
        v = torch.linspace(-4.0, 4.0, self.Nv, device=self.device)
        dt = t[1] - t[0]
        dv = v[1] - v[0]
        w_v = torch.ones(self.Nv, device=self.device) * dv
        w_v[0] *= 0.5
        w_v[-1] *= 0.5

        d_n = self.compute_d_n(initial_conditions)


        # basis_matrix = self.basis_matrix

        # # Reshape to (K, Nt, Nv) then take t=0 slice
        # basis_matrix_3d = basis_matrix.reshape(self.N_basis_t * self.N_basis_v, self.Nt, self.Nv)
        # basis_at_t0 = basis_matrix_3d[:, 0, :]  # (K, Nv)

        # d_n = integrate(w_v, basis_at_t0 @ initial_conditions)


        
        # Branch network outputs coefficients
        out = self.branch(x)  # (Batch, 2*K)
        A_real = out[..., :self.K*self.K].view(batch_size, self.K, self.K)   # (Batch, K)
        A_imag = out[..., self.K*self.K:].view(batch_size, self.K, self.K)   # (Batch, K)
        A = torch.complex(A_real,A_imag)

        c = torch.bmm(A, d_n.unsqueeze(-1)).squeeze(-1)

        # pred_real = self.basis_real @ c_real @ d_n # (Batch, 256)
        # pred_imag = self.basis_real @ c_imag @ d_n # (Batch, 256)

        basis = self.basis_real.to(c.dtype)
        pred = c @ basis
        
        return pred
    
    def training_step(self, batch, batch_idx):
        params, initial_conditions, solution = batch
        preds = self(params, initial_conditions)

        y_hat = preds.view(-1, self.Nt, self.Nv)
        y_true = solution.view(-1, self.Nt, self.Nv)

        # W = torch.ones(32, device=self.device)
        # diff_sq = (y_hat.real - y_true.real)**2 + (y_hat.imag - y_true.imag)**2
        # true_sq = y_true.real**2 + y_true.imag**2

        # l2_diff = integrate(W, diff_sq)**(0.5)
        # l2_norm = integrate(W, true_sq)**(0.5)
        
        # loss_back_up = torch.mean(l2_diff / torch.clamp(l2_norm, min=1e-7))

        loss = relative_l2_loss(y_hat,y_true)

        self.log("train_loss", loss)
        return loss
    
    def validation_step(self, batch, batch_idx):
        params, initial_conditions, solution = batch
        preds = self(params, initial_conditions)

        y_hat_val = preds.view(-1, self.Nt, self.Nv)
        y_true_val = solution.view(-1, self.Nt, self.Nv)

        # val_loss_org = self.loss_fn(preds.real, solution.real) + self.loss_fn(preds.imag, solution.imag)
        
        val_loss = relative_l2_loss(y_hat_val,y_true_val)
        self.log("val_loss", val_loss, prog_bar=True)
        return val_loss
    
    def configure_optimizers(self):
        return optim.Adam(self.parameters(), lr=1e-3)