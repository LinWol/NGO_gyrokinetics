...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! start input parameters !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  Ntsteps = 10000 ! number of time steps
  dt = 0.001 ! time step, should obey Courant limit: dt < dz/vmax
  ...
  epsilon_D = 0.5 ! diffusion term coefficient
...
! F0 already implemented
! F(t=0) initialized using the low level noise from the allocate function
  DO k = 1, Nz
    F(k,:)= 0.0001 * F0(:) * sin(k_par*z(k))
  END DO
...
CONTAINS
  ...
  SUBROUTINE kieval(ki,F)
    ...
    DO l = 1, Nv
    ki(1:Nz,l) = -vpar(l) * (-F(0:(Nz-1),l) + F(2:(Nz+1),l))*inv_2dz &
        -vpar(l) * F0(l) * (-Phi(0:(Nz-1)) + Phi(2:(Nz+1)))*inv_2dz &
        - (omega_n + (vpar(l)*vpar(l) - 0.5)*omega_Ti)*F0(l)*i*ky*Phi &
        + Di*(F(0:(Nz-1),l) - 2*F(1:(Nz),l) + F(2:(Nz+1),l))*inv_dz2 
    END DO
